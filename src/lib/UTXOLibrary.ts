...
import { logAttestationData, calculateAndValidateDepositHash } from './HashCalculator';
...
export class UTXOLibrary extends EventEmitter {
 ...
 async initialize(contractAddressOrProvider: string): Promise<boolean> {
  ...
  /**
   * Deposit ERC20 tokens as private UTXO using simplified implementation
   * @param params - Deposit parameters
   * @returns Promise resolving to operation result
   */
  async depositAsPrivateUTXO(params: CreateUTXOParams): Promise<UTXOOperationResult> {
    this.ensureInitialized();
    console.log(`💰 Creating private UTXO deposit using simplified implementation for ${params.amount} tokens...`);

    try {
      // Validate contract is available
      if (!this.contract) {
        throw new Error('❌ Contract not initialized. Please initialize the library first.');
      }
      ...
      console.log('🔀 Using simplified deposit implementation (no Zenroom dependencies)...');
      
      // Use the simplified implementation that guarantees no data regeneration
      return await depositAsPrivateUTXOSimplified(
        params,
        this.contract,
        this.currentEOA,
        this.ethereum,
